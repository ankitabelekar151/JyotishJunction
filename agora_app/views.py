...
def index(request):
    User = get_user_model()
    all_users = User.objects.exclude(id=request.user.id).filter(is_astrologer=True).only('id', 'username')
    return render(request, 'agora_app/index1.html', {'allUsers': all_users})
